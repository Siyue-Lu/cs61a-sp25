def shuffle(s):
    ...
    mid = len(s) // 2
    return [item for pair in zip(s[:mid], s[mid:]) for item in pair]
...
def deep_map(f, s):
    ...
    for i in range(len(s)):
        if isinstance(s[i], list): # supports subclasses, better than type(s[i]) == list
            deep_map(f, s[i])
        else:
            s[i] = f(s[i])
...
def balanced(m):
    ...
    def torque(end, arm):
        return total_mass(end) * length(arm)
    ...
    return (
        balanced(left_end)
        and balanced(right_end)
        and torque(left_end, left_arm) == torque(right_end, right_arm)
    )
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    return label(t) + max(max_path_sum(b) for b in branches(t)) # generator expression returns empty generator when t is leaf, process t manually
