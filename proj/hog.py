...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        roll = dice() # dice is non-pure, all rolls must be done even if there is already a 1
        if roll == 1:
            rolled_one = True
        if not rolled_one:
            total_score += roll
    return 1 if rolled_one else total_score
...
def boar_brawl(player_score, opponent_score):
    ...
    tens = opponent_score // 10 % 10 # getting exactly tens when opponent_score is greater than 100
    if ones == tens:
        return 1
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    strategies, scores = [strategy0, strategy1], [score0, score1] # initialise lists outside loop, shouldn't assign again
    while max(scores) < goal: # max(scores) < goal is same as both scores < goal
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    return 0 if boar_points >= threshold or sus_points(score + boar_points) - score >= threshold else num_rolls
