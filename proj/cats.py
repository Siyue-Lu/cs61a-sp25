...
def pick(paragraphs, select, k):
    ...
    ''
    """
    arr = [p for p in paragraphs if select(p)]
...
def about(keywords):
    ...
    return lambda p: any(k in split(lower(remove_punctuation(p))) for k in keywords)
...
def accuracy(typed, source):
    ...
    correct_count = sum(1 for i in range(min(typed_len, source_len)) if typed_words[i] == source_words[i])
    
    return (correct_count / typed_len) * 100.0
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        pair = (typed, source)
        if pair in cache:
            old_val, old_limit = cache[pair]
            if limit <= old_limit:
                return old_val
        # for not in cache or in but limit > old_limit
        new_val = diff_function(typed, source, limit)
        cache[pair] = (new_val, limit)
        return new_val
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    # apply key function on iterable, find the minimum key function result
    # throughout iterable, return corresponding iterable element produced the result
    closest_word = min(word_list, key=lambda word: diff_function(typed_word, word, limit))
    min_diff = diff_function(typed_word, closest_word, limit)
    return typed_word if min_diff > limit else closest_word
...
def furry_fixes(typed, source, limit):
    ...
    # if limit == 0:
    #     return 1
    # changed = 1 if typed[0] != source[0] else 0
    # return changed + furry_fixes(typed[1:], source[1:], limit - changed)

    def helper(typed, source, accu):
        # return accumulated sum when: amount of changing letters
        # exceeds limit; or the rest of typed and source are same
        if accu > limit or typed == source:
            return accu
        # runs out of typed or source, return accumulated sum +
        # length difference
        if not typed or not source:
            return accu + abs(len(typed) - len(source))
        # accu + 1 if initials are different
        changed = 1 if typed[0] != source[0] else 0
        return helper(typed[1:], source[1:], accu + changed)
...
def fastest_words(words_and_times):
    ...
    # each inner list represents the words that a player typed the fastest
    # index of the inner list corresponds to the index of the player in the times list
    fastest_list = [[] for _ in player_indices]

    # each tuple represents the time taken by each player to type the same word
    # index of the tuple corresponds to the index of the word in the words list
    word_times = list(zip(*times))

    for i in word_indices:
        # apply key function on iterable, find the minimum key function result
        # throughout iterable, return corresponding iterable element produced the result
        fastest_player = min(player_indices, key=lambda p: word_times[i][p])

        fastest_list[fastest_player].append(words[i])
