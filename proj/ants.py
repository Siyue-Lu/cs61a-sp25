...
class Ant(Insect):
    ...
    def double(self):
        if self.damage and not self.has_doubled:
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    # subclass instance attributes would be overwritten by default values in superclass init
    def __init__(self, health=1):
        super().__init__(health)
        # getting the class attributes of subclass, inheriting subclass' if not defined
        self.lower_bound = self.__class__.lower_bound
        self.upper_bound = self.__class__.upper_bound
    ...
    def nearest_bee(self):
        ...
        p = self.place
        ptr = 0
        while not p.is_hive and ptr <= self.upper_bound:
            if ptr >= self.lower_bound and p.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # copy list, avoid shortening original list and disrupting loop
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1):
        super().__init__(health)  # Ant's init needs health
        self.cooldown = 0
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # function attributes is unique feature of Python
        # def slowed_action(gamestate):
        #     if not slowed_action.cooldown or gamestate.time % 2 == 0:
        #         slowed_action.orig(gamestate)
        #     if slowed_action.cooldown:
        #         slowed_action.cooldown -= 1
        
        # if target:
        #     if not hasattr(target, "slowed"):
        #         target.slowed = True
        #         temp = target.action
        #         target.action = slowed_action
        #         target.action.orig = temp
        #     target.action.cooldown = self.slow_cooldown
        
        # object properties
        def slowed_action(self, gamestate):
            if not self.slow_cooldown or gamestate.time % 2 == 0:
                self.orig_action(gamestate)
            if self.slow_cooldown:
                self.slow_cooldown -= 1
        
        if target:
            if not hasattr(target, "slow_cooldown"):
    ...
    def insects_in_front(self):
        ...
        while not place.is_hive:
            for bee in place.bees:
                insect_dist[bee] = dist
            if place.ant and place.ant is not self:
                insect_dist[place.ant] = dist
            place = place.entrance
            dist += 1
        return insect_dist

    def calculate_damage(self, distance):
        dmg = self.damage - 0.25 * distance - 0.0625 * self.insects_shot
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if hasattr(self, "scare_cooldown") and self.scare_cooldown > 0:
            destination = self.place.entrance
            self.scare_cooldown -= 1
        else:
    ...
    def scare(self, length):
        ...
        """
        if not hasattr(self, "scare_cooldown"):
