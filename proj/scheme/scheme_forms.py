...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    seen_names = set() # value doesn't matter
    while bindings is not nil:
        binding = bindings.first
        # binding should be of form (name value), length exactly 2
        validate_form(binding, 2, 2)
        name = binding.first
        if not scheme_symbolp(name):
            raise SchemeError(f"name {name} must be a symbol")
        if name in seen_names: # O(1)
            raise SchemeError(f"multiple values for {name}")
        seen_names.add(name)
