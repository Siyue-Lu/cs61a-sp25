...
def insert_items(s, before, after):
    ...
    True
    """
    # s[:] = [x for item in s for x in ([item, after] if item == before else [item])]
    
    i = 0
    while i < len(s): # range(len(s)) only calculate len for once
        if s[i] == before:
            s.insert(i + 1, after)
            # s[:] = s[:i+1] + [after] + s[i+1:] # creating a new list every time
            i += 1
        i += 1
    return s
