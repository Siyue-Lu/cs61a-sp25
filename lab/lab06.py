class Transaction:
    ...
    def report(self):
        ...
        """
        if not self.changed():
            return f"{self.id}: no change"
        action = "increased" if self.after > self.before else "decreased"
        return f"{self.id}: {action} {self.before}->{self.after}"
...
class Coin:
    ...
    def worth(self):
        ...
        # return self.cents + (extra if extra > 0 else 0)
